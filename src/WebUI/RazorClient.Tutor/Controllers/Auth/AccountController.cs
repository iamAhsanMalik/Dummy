namespace RazorClient.Tutor.Controllers.Auth;
...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> Register(RegisterDTO model, string? returnUrl = null)
    {
        ...
        if (ModelState.IsValid)
        {
            var user = new ApplicationUser { UserName = $"{model.Email}1", Email = model.Email };
            ...
            if (registerResult?.Succeeded == true)
            {
                ...
                {
                    MailJetEmailRequest mailJetEmailRequest = new MailJetEmailRequest()
                    {
                        ToEmail = user.Email,
                        FromEmail = "AIU@aiu.com",
                        EmailSubject = "Confirm your email",
                        EmailBody = $"Please confirm your account by clicking here: <a href='{callbackUrl}'>Confirm Your Account</a>."
                    };
                    await _emailSender.MailJetEmailSenderAsync(mailJetEmailRequest);
                }
        ...
        if (ModelState.IsValid)
        {
            if (!string.IsNullOrEmpty(model.Email))
            {
                ...
                else
                {
                    ...
                    var emailbody = emailContent.Replace("$$ResetPasswordLink$$", callbackUrl).Replace("$$CurrentYear$$", DateTime.Now.Year.ToString());
                    MailJetEmailRequest mailJetEmailRequest = new MailJetEmailRequest()
                    {
                        ToEmail = user.Email,
                        FromEmail = "AIU@aiu.com",
    ...
    public async Task<IActionResult> SendCode(SendCodeDTO<SelectListItem> model)
    {
        ...
        if (!string.IsNullOrEmpty(model.SelectedProvider))
        {
            ...
            else if (model.SelectedProvider == "Phone")
            {
                ...
                {
                    await _smsSender.SMSSenderAsync(userPhone, message);
                }
            }
        }
